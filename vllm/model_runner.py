...
# Code adapted from https://github.com/maple-research-lab/SLOT
...
class GPUModelRunnerBase(ModelRunnerBase[TModelInputForGPU]):
    ...
    def __init__(
        ...
        # masking out all assistant-related tokens
        self.assistant_start_tokens = self.tokenizer.encode("assistant:")
        self.assistant_end_tokens = self.tokenizer.encode("user:")

        self.assistant_tool_call_start_tokens = self.tokenizer.encode("<tool_call>")
        self.assistant_tool_call_end_tokens = self.tokenizer.encode("</tool_call>")

        self.self_response_start_tokens = self.tokenizer.encode("<|im_start|>assistant")
        self.self_response_end_tokens = self.tokenizer.encode("<|im_end|>")

        self.user_start_tokens = self.tokenizer.encode("<|im_start|>user")
        self.user_end_tokens = self.tokenizer.encode("<|im_end|>")
        
        self.per_traj_loss_list = []
        self.all_loss_list = []
        # TODO: instead of using local file with atomic i/o for communication, we should use shared memory
        os.makedirs(os.path.dirname(self.all_loss_list_path), exist_ok=True)
        self.pa_envvar_path = f".tmp/{os.environ.get('TASK')}/pa_steps={os.environ.get('PA_STEPS', '-1')}_lr={os.environ.get('PA_LR', '0')}.txt"
        os.makedirs(os.path.dirname(self.pa_envvar_path), exist_ok=True)
        # Create the pa_envvar_path file if it does not exist
        if not os.path.exists(self.pa_envvar_path):
            with open(self.pa_envvar_path, "w") as f:
                f.write("1")  # create an empty file
        # ===== end of debugging ====
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            if tp_world_size == 1:
                ...
                tp_group.all_gather(tensor_list, local_head_shard)
    ...
    def profile_run(self) -> None:
        ...
        # this is to avoid ptuning params not being initialized
        self._reset_pa_envvars()
        self._dummy_run(max_num_batched_tokens, max_num_seqs)

    def _reset_pa_envvars(self):
        with open(self.pa_envvar_path, "w") as f:
            f.write("1")  # create an empty file
...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    ...
    def _find_token_patterns_vectorized(self, start_tokens: list[int], end_tokens: list[int], input_tokens: torch.Tensor):
        """
        Vectorized version using rolling window approach for better performance
        """
        positions = []
        
        # Convert to tensors once
        start_tokens_tensor = torch.tensor(start_tokens, device=input_tokens.device, dtype=input_tokens.dtype)
        end_tokens_tensor = torch.tensor(end_tokens, device=input_tokens.device, dtype=input_tokens.dtype)
        
        start_len = len(start_tokens)
        end_len = len(end_tokens)
        input_len = len(input_tokens)
        
        # Find all start positions using rolling window
        start_positions = []
        for i in range(input_len - start_len + 1):
            if torch.equal(input_tokens[i:i+start_len], start_tokens_tensor):
                start_positions.append(i)
        
        # For each start position, find the next end position
        for start_pos in start_positions:
            # Look for end_tokens starting from after start_tokens
            for j in range(start_pos + start_len, input_len - end_len + 1):
                if torch.equal(input_tokens[j:j+end_len], end_tokens_tensor):
                    end_pos = j + end_len - 1  # Include the full end token sequence
                    positions.append((start_pos, end_pos))
                    break
        
        return positions

    def _get_pa_reset_envvars(self):
        with open(self.pa_envvar_path, "r") as f:
            return f.read() == "1"
    ...
    def execute_model(
        ...
        if model_input.is_prompt:
            PA_STEPS = int(os.environ.get("PA_STEPS"))
            PA_LR = float(os.environ.get("PA_LR"))

            # initialize the parameters
            if self._get_pa_reset_envvars():
                ...
                self.adam_step = 0
                self.all_loss_list.append(self.per_traj_loss_list)
                with open(self.all_loss_list_path, "w") as f:
                    json.dump(self.all_loss_list, f, indent=2)
                self.per_traj_loss_list = []
                # end of initialization

            # ===== masking assistant tokens ====
            shift_labels = model_input.input_tokens[1:].contiguous()

            if self.mask_tokens == "true":
                # assistant_user_positions = self._find_token_patterns_vectorized(self.assistant_start_tokens, self.assistant_end_tokens, shift_labels)
                # assistant_tool_call_positions = self._find_token_patterns_vectorized(self.assistant_tool_call_start_tokens, self.assistant_tool_call_end_tokens, shift_labels)
                # assistant_response_positions = self._find_token_patterns_vectorized(self.self_response_start_tokens, self.self_response_end_tokens, shift_labels)
                user_positions = self._find_token_patterns_vectorized(self.user_start_tokens, self.user_end_tokens, shift_labels)

                # find the first position of user start tokens
                user_start_positions = self._find_token_patterns_vectorized(self.user_start_tokens, self.user_start_tokens, shift_labels)
                user_start_position = user_start_positions[0][0] if user_start_positions else 0

                # create mask for all positions
                mask = torch.ones_like(shift_labels, dtype=torch.float)
                for start, end in user_positions:
                    # we mask out user queries that are after system prompt
                    if start < user_start_position:
                        continue
                    mask[start:end+1] = 0.0 # we don't want to optimize on these tokens

                valid_label_mask = mask.float()

            # ===== end of masking assistant tokens ====
            ...
            for _ in range(PA_STEPS):
                ...
                if self.mask_tokens == "false":
                    valid_label_mask = (flat_labels >= 0).float() # BUG: this is not correct, because we are not using the padding tokens. The padding token id is also not negative.
                ...
                per_step_loss_list.append(loss.item())
                ...
                # Adam update
                self.adam_step += 1
                bias_correction1 = 1 - self.beta1**self.adam_step
                bias_correction2 = 1 - self.beta2**self.adam_step

                self.adam_m = self.beta1 * self.adam_m + (1 - self.beta1) * d_ptuning
                self.adam_v = self.beta2 * self.adam_v + (1 - self.beta2) * (d_ptuning**2)

                m_hat = self.adam_m / bias_correction1
                v_hat = self.adam_v / bias_correction2

                update = PA_LR * m_hat / (torch.sqrt(v_hat) + self.eps)
                self.ptuning_params = self.ptuning_params - update - PA_LR * self.weight_decay * self.ptuning_params
                del d_ptuning
            self.per_traj_loss_list.append(per_step_loss_list)
